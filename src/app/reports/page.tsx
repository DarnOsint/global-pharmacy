...
export default function ReportsPage() {
  ...
  const exportInventory = () => {
    const data = products.map(p => ({
      Name: p.name,
'SKU': p.sku, 'Product Code': p.product_code,
      Category: p.category,
  ...
  const exportAll = () => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(products.map(p => ({ Name: p.name, 'SKU': p.sku, 'Product Code': p.product_code, Stock: p.quantity_in_stock, Price: p.unit_price, Currency: p.currency, Expiry: p.expiry_date }))), 'Inventory');
