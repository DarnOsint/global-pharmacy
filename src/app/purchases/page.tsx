...
const mockPurchases = [
  { id: 'PO-2608-0001', supplier: 'Juba Pharma Ltd', items: [{ name: 'Paracetamol 500mg', qty: 500, cost: 1200, currency: 'SSP' as Currency }, { name: 'Amoxicillin 500mg', qty: 100, cost: 6000, currency: 'SSP' as Currency }], subtotal: 1200000, tax: 0, total: 1200000, status: 'received', created_at: '2026-08-10T10:00:00Z' },
  { id: 'PO-2608-0002', supplier: 'Medipharm Distributors', items: [{ name: 'Metformin 850mg', qty: 50, cost: 3.80, currency: 'USD' as Currency }, { name: 'Lisinopril 10mg', qty: 60, cost: 2.50, currency: 'USD' as Currency }], subtotal: 340, tax: 0, total: 340, status: 'ordered', created_at: '2026-08-15T14:30:00Z' },
  { id: 'PO-2608-0003', supplier: 'Bliss GVS Juba', items: [{ name: 'Artemether-Lumefantrine', qty: 200, cost: 10000, currency: 'SSP' as Currency }], subtotal: 2000000, tax: 0, total: 2000000, status: 'received', created_at: '2026-08-12T09:00:00Z' },
];

const statusColors: Record<string, 'success' | 'info' | 'danger'> = { received: 'success', ordered: 'info', cancelled: 'danger' };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card><CardContent>
            ...
            <p className="text-2xl font-bold text-success">{mockPurchases.filter(p => p.status === 'received').length}</p>
          </CardContent></Card>
          <Card><CardContent>
                ...
                {mockPurchases.map((po) => (
                  <tr key={po.id} className="border-b border-border hover:bg-muted/30">
                    ...
                    <td className="p-3 text-right font-semibold">
                      {po.items[0]?.currency === 'USD' ? formatCurrency(po.total, 'USD') : formatCurrency(po.total, 'SSP')}
                    </td>
                    ...
                    <td className="p-3 text-right">
                      <button onClick={() => { setSelected(po); setShowDetail(true); }} className="p-1.5 rounded hover:bg-muted"><Eye className="w-4 h-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody>
                {selected.items.map((item, i) => (
                  <tr key={i} className="border-b border-border/50">
                    ...
                    <td className="p-2 text-right">{formatCurrency(item.cost, item.currency)}</td>
                    <td className="p-2 text-right font-medium">{formatCurrency(item.qty * item.cost, item.currency)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
